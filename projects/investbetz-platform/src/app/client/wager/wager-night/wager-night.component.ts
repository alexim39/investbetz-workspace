...
@Component({
    selector: 'app-wager-night',
    templateUrl: './wager-night.component.html',
    styleUrls: ['./wager-night.component.scss'],
    animations: [
        trigger('fadeInOut', [
            state('void', style({
                opacity: 0
...
export class WagerNightComponent extends WagerClass implements OnInit {

  private clientId!: string;

  // Init odd values
  public eveningOdd!: number; // evening odd
  public eveningDate!: string;
  ...
  // inti countdown
  public eveningCountdown!: string;
  ...
  private loadTodayOdds() {
    // get morning odds
    this.wagerService.getEveningOdds().subscribe((response) => {
      if (response.message === 'done') {

        // Set odds to DOM
        this.eveningOdd = response.wager[0].odd;
        //this.eveningDate = response.wager[0].fullDate;
  ...
  // Evening Count down timer
  private eveningGameCountdown() {
    ...
    this.wagerService.getEveningOdds().subscribe((response) => {
      ...
      const x = setInterval(() => {
        ...
        this.eveningCountdown = days + 'd ' + hours + 'h ' + minutes + 'm ' + seconds + 's ';

        // If the count down is finished, write some text
        if (distance < 0) {
          clearInterval(x);
          this.eveningCountdown = 'ALREADY EXPIRED';
  ...
  public checkWagerAmount(wagerAmount: number) {
    ...
    this.wagerPayoutAmount = wagerAmount * this.eveningOdd;
    // Get amount after calculating payout percentage
    this.amountPayable = super.paybleAmount(this.wagerPayoutAmount);
  }

  // todd btn click
  public submitEveningOdd() {


    // Check if withdrawable = deposit amount
    /* if (this.toddValue > this.withdrawable) {
     this.terrMsg = 'Your withdrawable balance is less than your wager amount';
     this.tloading = false;
     // Remove msg after few sec
     setTimeout(() => { this.terrMsg = null; }, 6000);
     return;
    } */

    /* const wagerObj: WagerInterface = {
      wagerId: this.wagerId
    }; */

    // evening odd object
    const eveningWagerObj: InvestmntInterface = {
    ...
    this.wagerService.saveClientWager(eveningWagerObj).subscribe((response) => {
      if (response.message === 'done') {
        //this.successMessage = 'Your Evening Wager has been booked';
        this.snackBar.open('Your Evening Wager has been booked', 'Close', { 
          duration: 6000,
